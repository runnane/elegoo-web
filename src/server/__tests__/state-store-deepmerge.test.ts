/**
 * StateStore's delta merge (ELEG-91, slice 1 of ELEG-28).
 *
 * `deepMerge` in ../state-store.ts is private and unexported, so it is driven the way
 * the MQTT bridge actually drives it: `bridge.emit('status', delta)` → `handleStatusEvent`
 * → `deepMerge`. A refactor that bypasses `deepMerge` entirely would still be caught here,
 * which a direct unit test of the function would not catch.
 *
 * The invariant under test: **a field absent from a delta means unchanged, not cleared.**
 * Getting this wrong looks like flapping values in the UI, not a crash — see ELEG-28.
 *
 * Fixtures are generated, shaped like real CC2 status/canvas payloads (per the field names
 * in ../../types.js) but with no data captured from a real printer, per this repo's public
 * visibility.
 */
...
import type { PrinterStatus, CanvasInfo } from '../../types.js';
...
/** A full baseline status, shaped like a real CC2 method-1002 response. */
...
describe('StateStore status delta merge', () => {
  it('leaves a field absent from the delta unchanged — the core invariant', () => {
    const s = makeStore();
    bridge.emit('status', { result: baselineStatus() });

    // A real CC2 delta only ever carries the objects that changed — here, only extruder.
    bridge.emit('status', { result: { extruder: { temperature: 210.1 } } });

    expect((s.status as PrinterStatus).extruder.temperature).toBe(210.1);
    // heater_bed was entirely absent from the delta: it must be untouched, not cleared.
  ...
  it('merges a nested object rather than replacing it wholesale', () => {
    ...
    // Delta only names extruder.temperature; extruder.target is absent from the delta's
    // own nested object, not just the top-level payload.
    bridge.emit('status', { result: { extruder: { temperature: 212 } } });
  ...
  it('replaces an array wholesale rather than merging it element-wise', () => {
    ...
    bridge.emit('status', {
      result: { machine_status: { exception_status: [5001] } },
    });

    expect(s.status?.machine_status.exception_status).toEqual([5001]);
    // sibling fields in the same nested object are still preserved
  ...
  it('applies an explicit falsy value from the delta rather than treating it as absent', () => {
    ...
    expect(s.status?.heater_bed.temperature).toBe(59.8); // sibling untouched
  ...
  it('applies an explicit null from the delta as a value, not an absence', () => {
    ...
    bridge.emit('status', {
      result: { print_status: { state: null } },
    });
  ...
  it('merges canvas_info deltas the same way, leaving absent fields unchanged', () => {
    const s = makeStore();
    bridge.emit('status', { result: { ...baselineStatus(), canvas_info: baselineCanvas() } });

    // Only active_tray_id changes (a tray swap); canvas_list and auto_refill are absent.
    bridge.emit('status', {
      result: { canvas_info: { active_tray_id: 1 } },
    });
